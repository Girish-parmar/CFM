...
One lab per technical module. Each lab is a Python script in Jupytext "percent" format: run it as a script, or open it as a notebook in JupyterLab/VS Code (`pip install jupytext`, then *Open With → Notebook*). Charts are saved to `labs/output/`.
...
Every lab runs offline on synthetic or fictional data. Lab 2 can switch to real data (`USE_REAL_DATA = True`, needs `pip install yfinance`). Lab 14's LLM step runs only when `ANTHROPIC_API_KEY` is set (`pip install anthropic`); it uses `claude-opus-5` unless `CFMAT_CLAUDE_MODEL` names another model.
