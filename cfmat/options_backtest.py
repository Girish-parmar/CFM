...
def backtest_options(
    ...
    for expiry in expiries:
        ...
        widths = _max_loss(legs, strikes, net, lot_size)
        n_short = sum(1 for _, _, qty in legs if qty < 0)
        margin = widths if np.isfinite(widths) else n_short * margin_rate * s_arr[i0] * lot_size / max(1, n_short) + credit
